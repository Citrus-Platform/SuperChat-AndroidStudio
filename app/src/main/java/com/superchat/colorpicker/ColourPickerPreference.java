...
public class ColourPickerPreference extends Preference {
	private final boolean supportsAlpha = false;
	int value;
...
//		supportsAlpha = ta.getBoolean(R.styleable.AmbilWarnaPreference_supportsAlpha, false);
	...
	@Override protected void onClick() {
		new ColourPickerDialog(getContext(), value, supportsAlpha, new ColourPickerDialog.OnAmbilWarnaListener() {
			@Override public void onOk(ColourPickerDialog dialog, int color) {
			...
			@Override public void onCancel(ColourPickerDialog dialog) {
				// nothing to do
			}
		}).show();
	}
